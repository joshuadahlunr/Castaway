...
public class MonsterDatabase : ScriptableObject {
	...
	public struct MonsterData {
		...
		public MonsterCardBase PopulateDeck(MonsterCardBase instantiatedMonsterToPopulate) {
			...
			foreach(var card in deckList)
				instantiatedMonsterToPopulate.deck.AddCard(Instantiate(card)); // We instantiate the card here to allow prefabs to be passed in (non prefabs should just get copied)
	...
	/// </summary>
	public MonsterDictionary cards = new MonsterDictionary();

	// Instantiate a card in the database and return a reference to its CardBase
	public MonsterCardBase Instantiate(string cardName) =>
		cards.TryGetValue(cardName, out var m) ? m.PopulateDeck(Instantiate(m.monsterCard).GetComponent<MonsterCardBase>()) : null;
	public MonsterCardBase Instantiate(string cardName, Transform parent)
		=> cards.TryGetValue(cardName, out var m) ? m.PopulateDeck(Instantiate(m.monsterCard, parent).GetComponent<MonsterCardBase>()) : null;
	public MonsterCardBase Instantiate(string cardName, Vector3 position, Quaternion rotation)
		=> cards.TryGetValue(cardName, out var m) ? m.PopulateDeck(Instantiate(m.monsterCard, position, rotation).GetComponent<MonsterCardBase>()) : null;
	public MonsterCardBase Instantiate(string cardName, Vector3 position, Quaternion rotation, Transform parent)
		=> cards.TryGetValue(cardName, out var m) ? m.PopulateDeck(Instantiate(m.monsterCard, position, rotation, parent).GetComponent<MonsterCardBase>()) : null;
}
