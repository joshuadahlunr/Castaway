...
namespace CardBattle {
	...
	public struct HealthState {
		...
		public HealthState ApplyDamage(int damage) {
			...
			temp = Math.Max(damage - temporaryDamageReduction,
				0); // subtract temporary damage reduction from damage and set temp to the maximum of the result and 0
			temporaryDamageReduction =
				Math.Max(temporaryDamageReduction - damage,
					0); // subtract damage from temporary damage reduction and set temporaryDamageReduction to the maximum of the result and 0
			damage = temp; // set damage to temp

			temp = Math.Max(damage - permanentDamageReduction,
				0); // subtract permanent damage reduction from damage and set temp to the maximum of the result and 0
			permanentDamageReduction =
				Math.Max(permanentDamageReduction - damage,
					0); // subtract damage from permanent damage reduction and set permanentDamageReduction to the maximum of the result and 0
			damage = temp; // set damage to temp
