...
namespace CardBattle {
	...
	public static class CardFilterer {
		...
		/// <returns>A tuple containing two arrays of cards: the first contains the cards that match the filters,
		/// and the second contains the cards that were filtered out.</returns>
		private static (Card.CardBase[], Card.CardBase[]) FilterCardsImpl(CardFilters filters, IEnumerable<Card.CardBase> cards) {
			...
			foreach (var card in cards) {
			    ...
			    if ((filters & CardFilters.Hand) != 0) {
			        if ((card.state & Card.CardBase.State.InHand) != 0) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
			    ...
			    if ((filters & CardFilters.InPlay) != 0) {
			        if ((card.state & Card.CardBase.State.InPlay) != 0) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
			    ...
			    if ((filters & CardFilters.MonsterImpl) != 0) {
			        if (card is Card.MonsterCardBase) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
			    ...
			    if ((filters & CardFilters.EquipmentImpl) != 0) {
			        if (card is Card.EquipmentCardBase) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
			    ...
			    if ((filters & CardFilters.Action) != 0) {
			        if (card is Card.ActionCardBase) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
			    ...
			    if ((filters & CardFilters.Status) != 0) {
			        if (card is Card.StatusCardBase) {
			            filteredCards.Add(card);
			            continue;
			        }
			    }
		...
		/// <param name="filters">The filters to use for the card filtering.</param>
		/// <returns>An array of the filtered cards that were not disabled.</returns>
		public static Card.CardBase[] FilterAndDisableCards(CardFilters filters) =>
